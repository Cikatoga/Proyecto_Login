...
class App(ctk.CTk):
    ...
    def mostrar_toast(self, mensaje):
        ...
        ctk.CTkLabel(toast, text=mensaje, text_color="white", font=("Roboto", 11, "bold"), wraplength=300).pack(expand=True, padx=10, pady=5)
    ...
    def logic_favoritos(self, peli_datos):
        # Aseguramos que sea una lista para poder contarla
        favs_actuales = obtener_favoritos(self.usuario_actual)
        if not isinstance(favs_actuales, list):
            favs_actuales = []
            
        if len(favs_actuales) >= 10:
            msg_limite = "¡Oh no! has llegado al límite de favoritos. PD: Sabemos que es difícil cumplir el objetivo, pero vamos, son solo 10 películas"
            self.mostrar_toast(msg_limite)
        else:
            exito, msg = agregar_a_favoritos(self.usuario_actual, peli_datos)
            self.mostrar_toast(msg)
